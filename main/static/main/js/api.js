...
async function apiRequest(endpoint, method = 'GET', data = null, requiresAuth = false) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            // Check for user-friendly error message first
            if (responseData.error) {
                ...
                if (typeof responseData.errors === 'object') {
                    ...
                    errorMessage = errorList.join('; ') || 'Please correct the errors in the form.';
                } else {
                ...
                errorMessage = values.length > 0 ? values.join(', ') : `Server error: ${response.status} ${response.statusText}`;
            } else {
